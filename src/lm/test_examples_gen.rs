...
fn test_linear_full_rank() {
    ...
    assert_fp_eq!(problem.params, VectorN::<f64, U5>::from_column_slice(&[-0.9999999999999953, -1.0000000000000049, -0.9999999999999976, -0.9999999999999956, -0.9999999999999991]));
}
...
fn test_linear_rank1() {
    ...
    assert_fp_eq!(problem.params, VectorN::<f64, U5>::from_column_slice(&[-20.29999900022674, -9.64999950011337, -165.2451975264496, -4.324999750056676, 110.53305851006517]));
}
...
fn test_linear_rank1_zero_columns() {
    ...
    assert_fp_eq!(problem.params, VectorN::<f64, U5>::from_column_slice(&[1., 332.1494858957815, -439.6851914289522, 163.69688258258626, 1.]));
}

#[test]
fn test_rosenbruck() {
    let mut problem = Rosenbruck { params: VectorN::<f64, U2>::zeros() };
...
fn test_helical_valley() {
    let mut problem = HelicalValley { params: VectorN::<f64, U3>::zeros() };
    ...
    assert_fp_eq!(problem.params, VectorN::<f64, U3>::from_column_slice(&[1., -1.9721522630525295e-30, 0.]));
}

#[test]
fn test_powell_singular() {
    let mut problem = PowellSingular { params: VectorN::<f64, U4>::zeros() };
    ...
    assert_fp_eq!(problem.params, VectorN::<f64, U4>::from_column_slice(&[3.2267921800163781e-18, -3.2267921800163780e-19, 5.1628674880262125e-19, 5.1628674880262125e-19]));
}

#[test]
fn test_freudenstein_roth() {
    let mut problem = FreudensteinRoth { params: VectorN::<f64, U2>::zeros() };
    ...
    assert_fp_eq!(problem.params, VectorN::<f64, U2>::from_column_slice(&[11.412781785788198, -0.8968051074920677]));
}

#[test]
fn test_bard() {
    let mut problem = Bard { params: VectorN::<f64, U3>::zeros() };
    ...
    assert_fp_eq!(problem.params, VectorN::<f64, U3>::from_column_slice(&[8.4066667386764549e-01, -1.5894616720551842e+08, -1.6446490685777116e+08]));
}

#[test]
fn test_kowalik_osborne() {
    let mut problem = KowalikOsborne { params: VectorN::<f64, U4>::zeros() };
    ...
    assert_fp_eq!(problem.params, VectorN::<f64, U4>::from_column_slice(&[0.19279840638465487, 0.1914736844615448, 0.1230924753714115, 0.13615096290622444]));
}

#[test]
fn test_meyer() {
    let mut problem = Meyer { params: VectorN::<f64, U3>::zeros() };
